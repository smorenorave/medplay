...
function Td(props: TdHTMLAttributes<HTMLTableCellElement>) {
  ...
  return (
    <td
      {...rest}
      className={[
        "px-3 py-2 text-sm text-neutral-100 whitespace-nowrap",
        className,
      ].join(" ")}
    >
      {children}
...
function PlataformasPane() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const list: Plataforma[] = (Array.isArray(data) ? data : []).map(
        mapPlatform,
      );
      list.sort((a, b) =>
  ...
  const onCreate = async (e: FormEvent) => {
    ...
    if (!Number.isInteger(nCant) || nCant < 0)
      return setFormErr("Cantidad de pantallas debe ser entero ≥ 0.");
    setFormErr(null);
    setCreating(true);
    try {
      ...
      setFormErr(e?.message ?? "Error al crear");
    } finally {
      setCreating(false);
    }
  };
  ...
  const saveEdit = async () => {
    ...
    if (!Number.isInteger(nCant) || nCant < 0)
      return setSaveErr("Cantidad de pantallas debe ser entero ≥ 0.");
    setSaving(true);
    setSaveErr(null);
    try {
  ...
  const onDelete = async (row: Plataforma) => {
    ...
    try {
      const res = await fetch(`/api/plataformas/${row.id}`, {
        method: "DELETE",
      });
      if (res.ok) {
      ...
      if (res.status === 409) {
        alert(
          j?.message ||
            "No se puede eliminar: la plataforma tiene registros asociados.",
        );
        return;
      }
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) =>
      tokens.every((t) =>
        norm(`${r.nombre} ${r.cantidad_pantallas}`).includes(t),
      ),
    );
  }, [rows, q]);
  ...
  return (
    <Card>
      ...
      <div className="grid gap-2 sm:grid-cols-[1fr_120px_150px_170px_auto] px-3 py-3 sm:px-4">
        <input
          className="h-10 w-full rounded-lg px-3 border border-neutral-700 bg-neutral-900 text-neutral-100 outline-none focus:ring-2 focus:ring-neutral-600 focus:border-neutral-500"
        ...
        <button
          type="button"
          onClick={load}
          className="h-10 rounded-lg border border-neutral-700 bg-neutral-900 px-3 text-neutral-100 hover:bg-neutral-800"
        >
          Refrescar
        </button>
      </div>
                      ...
                      {!isEditing ? (
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            ...
                            className="inline-flex items-center justify-center rounded-md p-2 text-red-200 hover:bg-red-800/30 hover:text-red-100 focus:outline-none focus:ring-2 focus:ring-red-600"
                            title="Eliminar"
                          >
                            <TrashIcon />
                          </button>
...
function ContactosPane() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      const list: Contacto[] = (Array.isArray(data) ? data : []).map(
        (u: any, i: number) => ({
          id: Number(u?.id ?? i + 1),
  ...
  const onCreate = async (e: FormEvent) => {
    e.preventDefault();
    const c = nuevoContacto.trim(),
      n = nuevoNombre.trim();
    if (!c) return setFormErr("Escribe un contacto.");
    setFormErr(null);
    setCreating(true);
    try {
      const payload: any = { contacto: c };
      if (n.length > 0) payload.nombre = n;
      ...
      setRows((rs) => [
        ...rs,
        {
          ...
          contacto: String(saved.contacto),
  ...
  const beginEdit = (row: Contacto) => {
    setEditingId(row.id);
    setDraft({ ...row, nombre: row.nombre ?? "" });
    setSaveErr(null);
  };
  const cancelEdit = () => {
    setEditingId(null);
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/usuarios/${encodeURIComponent(current.contacto)}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
      ...
      if (!res.ok) {
        if (res.status === 409 && upd?.error === "contacto_duplicado")
      ...
      setRows((rs) =>
        rs.map((r) =>
          r.id === editingId
            ? {
                ...r,
                contacto: String(upd?.contacto ?? c),
                nombre: (upd?.nombre ?? nRaw) || null,
              }
            : r,
        ),
      );
  ...
  const onDelete = async (row: Contacto) => {
    ...
    try {
      const res = await fetch(
        `/api/usuarios/${encodeURIComponent(row.contacto)}`,
        { method: "DELETE" },
      );
      if (res.ok) {
        setRows((rs) => rs.filter((r) => r.contacto !== row.contacto));
        return;
      }
      const j = await res.json().catch(() => ({}) as any);
      if (res.status === 409) {
  ...
  const filtered = useMemo(() => {
    ...
    if (!tokens.length) return rows;
    return rows.filter((r) =>
      tokens.every((t) => norm(`${r.contacto} ${r.nombre ?? ""}`).includes(t)),
    );
  }, [rows, q]);
  ...
  return (
    <Card>
      ...
      <div className="px-3 pt-3 sm:px-4">
        ...
        >
          <div className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]">
            <input
              className="h-10 w-full rounded-lg px-3 border border-neutral-700 bg-neutral-900 text-neutral-100 outline-none focus:ring-2 focus:ring-neutral-600 focus:border-neutral-500"
              placeholder="Contacto (+57 3xx … o texto)"
              value={nuevoContacto}
      ...
      <div className="grid gap-2 sm:grid-cols-[1fr_auto] px-3 py-3 sm:px-4">
        <input
          className="h-10 w-full rounded-lg px-3 border border-neutral-700 bg-neutral-900 text-neutral-100 outline-none focus:ring-2 focus:ring-neutral-600 focus:border-neutral-500"
          placeholder="Buscar contacto o nombre…"
          value={q}
          onChange={(e) => setQ(e.target.value)}
        />
        <button
        ...
        {!err && (
          <table className="min-w-[680px] w-full table-fixed">
            ...
            <tbody>
              ...
              {filtered.length === 0 && (
                <tr>
                  <Td colSpan={3} className="text-neutral-300 text-sm py-4">
                    No hay resultados.
                  </Td>
                </tr>
              )}
...
function InventarioPane() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setPlatErr(e?.message ?? "Error al cargar plataformas");
      } finally {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      const list: Inventario[] = (Array.isArray(data) ? data : []).map(
        (r: any) => ({
          id: Number(r.id),
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      setCreateErr(e?.message ?? "Error al guardar");
    } finally {
      setCreating(false);
    }
  }
  ...
  /* -------- edición -------- */
  const beginEdit = (row: Inventario) => {
    setEditingId(row.id);
    setDraft({ ...row });
    setSaveErr(null);
  };
  ...
  const filteredClient = useMemo(() => {
    ...
    return rows.filter((r) => {
      const h = norm(
        `${platformMap.get(r.plataforma_id) ?? r.plataforma_id} ${r.correo} ${r.clave ?? ""}`,
      );
  ...
  return (
    <Card>
      ...
      <div className="px-3 pb-4 pt-3 sm:px-4 space-y-4">
        ...
        <form onSubmit={onCreate} className="grid gap-3 sm:grid-cols-3">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setPlataformaId(e.target.value ? Number(e.target.value) : "")
              }
        ...
        {createErr && (
          <div className="text-sm text-red-300">Error: {createErr}</div>
        )}
        ...
        <div className="grid gap-2 sm:grid-cols-[minmax(180px,280px)_1fr_auto]">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setFPlataformaId(e.target.value ? Number(e.target.value) : "")
              }
              disabled={loadingPlat || !!platErr}
            >
        ...
        <div className="flex flex-wrap items-center gap-2 justify-between border border-neutral-800 rounded-xl px-3 py-2 bg-neutral-950/40">
          ...
          <div className="flex items-center gap-2">
            <button onClick={goFirst} className={btn} disabled={page <= 1}>
              « Primero
            </button>
            <button onClick={goPrev} className={btn} disabled={page <= 1}>
              ‹ Anterior
            </button>
        ...
        <div className="max-h-[48vh] overflow-auto custom-scroll">
          ...
          {!err && (
            <table className="min-w-[840px] w-full table-fixed">
              ...
              <tbody>
                {pageRows.map((row, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Td>
                        {!isEditing ? (
                          (platformMap.get(row.plataforma_id) ??
                          row.plataforma_id)
                        ) : (
                          <select
                            className={[
                              input,
                              "h-9 py-1 [&>option]:bg-neutral-900 [&>option]:text-neutral-100",
                            ].join(" ")}
                            value={String(
                              draft.plataforma_id ?? row.plataforma_id,
                            )}
                            onChange={(e) =>
                              setDraft((d) => ({
                                ...d,
                ...
                {pageRows.length === 0 && (
                  <tr>
                    <Td colSpan={4} className="text-neutral-300 text-sm py-4">
                      No hay resultados.
                    </Td>
                  </tr>
                )}
          ...
          {saveErr && (
            <div className="m-3 rounded-lg border border-red-800/50 bg-red-950/30 p-2 text-sm text-red-200">
              {saveErr}
            </div>
          )}
        </div>
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-semibold text-neutral-100">
        Plataformas, Contactos e Inventario
      </h2>
      {/* Tabs (mismo nivel) */}
      <div className="flex gap-2">
        <button
          className={`${btnBase} ${tab === "plataformas" ? active : idle}`}
          onClick={() => setTab("plataformas")}
        >
          Plataformas
        </button>
        <button
          className={`${btnBase} ${tab === "contactos" ? active : idle}`}
          onClick={() => setTab("contactos")}
        >
          Contactos
        </button>
        <button
          className={`${btnBase} ${tab === "inventario" ? active : idle}`}
