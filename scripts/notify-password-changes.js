...
const FIRST_BOOT_PASSIVE_WAIT_MS = 390_000; // 6.5 min aprox (ajustado)
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      child.stdout.on("data", (b) =>
        flog(`[start-sh][out] ${String(b).trim()}`)
      );
      child.stderr.on("data", (b) =>
        flog(`[start-sh][err] ${String(b).trim()}`)
      );
...
async function waitForWhatsAppReady(
  page,
  { timeout = 120_000, quietMs = 2000, requireWsTraffic = true } = {}
) {
  ...
  ctx.on("serviceworker", (sw) => {
    if (String(sw.url || "").startsWith("https://web.whatsapp.com/"))
      hasWASW = true;
  });
  ...
  try {
    ...
    while (Date.now() - t0 < timeout) {
      if (hasWASW || wsOpen) {
        const quietOk = await waitForNetworkQuiet(page, {
          quietMs,
          timeout: 5000,
        });
...
async function ensureChatReady(page, phone, textEncoded) {
  ...
  for (let attempt = 1; attempt <= CHAT_RETRIES; attempt++) {
    ...
    flog(
      `🔁 [STEP 7] Abriendo chat (intento ${attempt}/${CHAT_RETRIES}) con: ${urlToOpen}`
    );
    await page
      .goto(urlToOpen, { waitUntil: "domcontentloaded", timeout: 60_000 })
      .catch(() => {});
    ...
    if (!resolved) {
      flog(
        "⚠️ Deep link no resolvió; reintento con otra variante (sin hard reset)…"
      );
...
function buildMessage(nombre, item, correo, nuevaClave) {
  ...
  const base = `• ${item.plataforma_nombre || "tu plataforma"} — ${
    item.servicio
  }`;
  const extra =
    item.servicio === "Pantalla" && item.nro_pantalla
      ? ` | *Pantalla ${item.nro_pantalla}*`
      : "";
  const vence = item.fecha_vencimiento
...
(async function main() {
  ...
  try {
    ...
    try {
      ...
      flog(
        `[STEP 3] ❌ Error fetchByCorreosAndPlataformas: ${e?.message || e}`
      );
      throw e;
    }
    ...
    const bump = (k, platName, phone) => {
      if (!perCorreoPlat.has(k))
        perCorreoPlat.set(k, {
          plataforma_nombre: platName || "N/D",
          count: 0,
          phones: new Set(),
        });
    ...
    tasks.sort((a, b) => {
      ...
      if (pn !== 0) return pn;
      return (
        a.plataforma_id - b.plataforma_id || a.phone.localeCompare(b.phone)
      );
    });
    ...
    for (const [k, v] of perCorreoPlat.entries()) {
      ...
      flog(
        `[STEP 4][RESUMEN] correo=${corr} plataforma_id=${pid} (${
          v.plataforma_nombre
        }) filas=${v.count} phones=${Array.from(v.phones).join(",")}`
      );
    }
    ...
    if (!tasks.length) {
      flog(
        "⛔ No hay tareas tras filtros (fechas futuras / correos / teléfonos / plataforma) → salida temprana."
      );
      console.log(
        "No hay destinatarios (verifica fechas futuras, correos, teléfonos y plataforma)."
      );
      return;
    }
    ...
    if (!ok) {
      flog(
        "[STEP 6.2] Falló espera de CDP. Reintento único: relanzo .sh y espero 25s más…"
      );
      await launchChromeViaSh();
    ...
    flog(
      `⏳ [STEP 6.3] Warmup inicial: esperando ${Math.round(
        FIRST_BOOT_PASSIVE_WAIT_MS / 1000
      )}s fijos (sin recargar)…`
    );
    ...
    console.log(
      `Notificando ${tasks.length} mensaje(s) en orden por correo y plataforma…`
    );
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      if (blkKey !== prevKey) {
        flog(
          `— BLOQUE — correo=${t.correo} | plataforma_id=${t.plataforma_id} (${
            t.plataforma_nombre || "N/D"
          })`
        );
        prevKey = blkKey;
      }
      ...
      flog(
        `→ [${i + 1}/${tasks.length}] phone=${t.phone} correo=${
          t.correo
        } plataforma_id=${t.plataforma_id} (${
          t.plataforma_nombre || "N/D"
        }) servicio=${t.item.servicio} pantalla=${
          t.item.nro_pantalla || "—"
        } vence=${fmtDateDDMMYYYY(t.item.fecha_vencimiento) || "—"}`
      );
      ...
      if (i < tasks.length - 1) {
        ...
        flog(
          `⏳ Pausa entre mensajes: ~${Math.round(
            pause / 1000
          )}s (incluye jitter)…`
        );
        await sleep(pause);
